...
for (nu_true in NUS) {
  ...
  for (n in NS) {
    ...
    for (r in seq_len(R)) {
      ...
      # Cache this fit's model-selection criteria (needs ff=0, same as
      # what this loop already fits with) for Part 4 to reuse -- if this
      # fails for some numerical reason, just skip caching (Part 4 falls
      # back to fitting CMP fresh for this rep), don't kill the recovery
      # loop over an optional step.
      crit_cmp <- tryCatch(get_mod_sel_cmp(y, fit, M, HYBRID_TOL),
                           error = function(e) NULL)
      cmp_cache[[key]][[r]] <- list(y = y, crit_cmp = crit_cmp)
...
for (nu_true in CMP_NUS) {
  ...
  for (n_val in CMP_NS) {
    ...
    for (r in seq_len(CMP_R)) {
      ...
      if (is.null(fit_poi) || is.null(fit_nb) || is.null(fit_gp)) next
...
cat(sprintf("\n[Part 4] CMP reused from Part 1's cache: %d reps  |  CMP fit fresh here: %d reps\n",
            n_reused, n_fresh))
