...
# Uses YOUR ZIINAR1-CMP-fast.stan as-is (alpha, lambda, nu, rho; student_t
# priors on lambda/nu as coded there). Produces the table for
# Section 4.2 (Table \ref{tab:sim_placeholder}) plus two independent
# comparators (CLS for alpha; a moment-based estimator for lambda,nu given
# the true alpha,rho) so the Bayesian numbers aren't the only check.
#
# HOW TO USE
#   1. Put this file and ZIINAR1-CMP-fast.stan in the same directory.
#   2. First run with QUICK_TEST <- TRUE to confirm the model compiles and
#      one cell runs in a sane amount of time on your machine.
#   3. Set QUICK_TEST <- FALSE, adjust R / ITER / WARMUP for your compute
#      budget, and run: Rscript run_simulation_grid.R
#   4. Results checkpoint after every (nu, n) cell to sim_results_partial.rds
#      so a long run can be killed and resumed without losing progress.
#   5. Final outputs: sim_summary_table.csv (numbers) and
...
STAN_FILE   <- "ZIINAR1-CMP-fast.stan"
M           <- 300
HYBRID_TOL  <- 1e-6
...
for (nu_true in NUS) {
  ...
  for (n in NS) {
    ...
    for (r in seq_len(R)) {
      ...
      cat(sprintf("  rep %d/%d  Rhat_ok=%s  alpha=%.3f  lambda=%.3f  nu=%.3f  rho=%.3f  (cls_alpha=%.3f, mom_nu=%.3f)\n",
                  r, R, rhat_ok, s["alpha", "mean"], s["lambda", "mean"],
                  s["nu", "mean"], s["rho", "mean"], a_cls, unname(mom["nu"])))
    }
